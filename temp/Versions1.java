...
public class Versions {
    ...
    public static class V1 extends Scales implements ScaleInterface {
        ...
        public boolean load() throws Exception{ //загрузить данные
            try {
                filter=Integer.valueOf(command(FILTER));
                if(filter<0 || filter>15) {
                    if(!command(FILTER+8).equals(FILTER))
            ...
            try {
                timer=Integer.valueOf(command(Scales.TIMER));
                if(timer<1 || timer>60) {
                    if (!command(TIMER+10).equals(TIMER))
            ...
            try {
                speed=Integer.valueOf(command(Scales.SPEED));
                if(speed<1 || speed>5) {
                    if (!command(SPEED+5).equals(SPEED))
            ...
            if(!isDataValid(command(DATA)))
                if(Preferences.contains("data"+getAddress())) {
                    data=Preferences.read("data"+getAddress(),"");
                    if(!isDataValid(data))
                        return false;
                    //command(DATA+data);
                }
            weightError=(int)(weightMax*0.001);
    ...
    public static class V4 extends Scales implements ScaleInterface {
        ...
        public boolean load() throws Exception { //загрузить данные
            ...
            return true;
            //throw new Exception("Установить настройки ограничений");//todo временно для отладки
            //return false;//todo временно для отладки
        }
